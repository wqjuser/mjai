...
class MusicSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _buildSectionTitle('音乐生成设置', settings),
        const SizedBox(height: 16),

        // Suno API配置卡片
        _buildSunoConfigCard(settings),
      ],
    );
  }
  ...
  // Suno API配置卡片
  Widget _buildSunoConfigCard(ChangeSettings settings) {
            ...
            children: [
              Expanded(
                child: _buildTextField(
                  controller: state.getTextController('suno_api_url'),
                  label: 'API地址',
              ...
              Expanded(
                child: _buildTextField(
                  controller: state.getTextController('suno_api_key'),
                  label: 'API Key',
                  onChanged: (text) async {
